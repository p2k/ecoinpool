...
init([SubpoolId, Config]) ->
    ...
    {ok, #state{subpool=SubpoolId, pool_type=PoolType, url=URL, auth={User, Pass}, tag=FullTag, pay_to=PayTo, ebtc_id=EBtcId, txtbl=TxTbl, worktbl=WorkTbl}}.
...
handle_cast(post_workunit, OldState) ->
    ...
    #state{subpool=SubpoolId, tag=Tag, pay_to=PubkeyHash160, worktbl=WorkTbl, block_num=BlockNum, memorypool=Memorypool, coinbase_tx=OldCoinbaseTx} = State,
